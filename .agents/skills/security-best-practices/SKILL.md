...
description: "Perform JavaScript/TypeScript and React frontend security reviews and suggest improvements for FStructure. Trigger only when the user explicitly requests security guidance, a security review/report, or secure-by-default coding help. Do not trigger for general code review, debugging, or non-security tasks."
---

# Security Best Practices

> Adapted for FStructure from OpenAI's `security-best-practices`: only the JavaScript frontend and React/TypeScript references are distributed.
...
The initial step is to confirm that the task concerns the JavaScript/TypeScript React frontend present in FStructure. Rust/WASM, Python tooling and infrastructure require their own primary guidance; do not improvise framework-specific rules from absent references.
...
For FStructure, read both `javascript-general-web-frontend-security.md` and `javascript-typescript-react-web-frontend-security.md`. Apply `AGENTS.md` and the local-first/privacy invariants before generic recommendations.
