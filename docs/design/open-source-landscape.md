...
| CalculiX CrunchiX 2.23 | Ejecutable nativo `ccx_2.23`; el `Makefile` compila fuentes C con `cc` y Fortran con `gfortran`, y enlaza SPOOLES/ARPACK en la configuración distribuida. | El encabezado fuente lo define como programa de elementos finitos 3D. El tarball contiene rutinas de vigas y shells; no se verificó un solver 2D autónomo. | El snapshot contiene entry points/rutinas para estática, dinámica, modal/frecuencia, estado estable y transferencia térmica; esta constatación de código fuente no sustituye validación numérica. | Los encabezados de `CalculiX.h` y `ccx_2.23.c` declaran GNU GPL versión 2 o, según `CalculiX.h`, cualquier versión posterior. | Sólo referencia comparativa. No se adopta, integra, añade como dependencia ni usa como oráculo en el plan aprobado. |
...
1. OpenSees/OpenSeesPy, XC, Frame3DD y CalculiX CrunchiX son motores o marcos de análisis; ninguno vuelve automáticamente verificable una cláusula de diseño CDMX.
...
## Evidencia primaria reproducible

Las afirmaciones de la tabla se apoyan en revisiones Git inmutables o, para CalculiX, en el archivo fuente oficial versionado cuya huella SHA-256 fija los bytes revisados. Una página viva no sustituye esa evidencia.

| Herramienta | Revisión fijada | Evidencia reproducible de capacidades/licencia |
| --- | --- | --- |
...
| CalculiX | CrunchiX `2.23`, snapshot oficial SHA-256 `9c88385c10fb04f5dc6c4e98027a51bebdd8aee3920e05190d6c1dd08357d6e7` | [Archivo fuente oficial versionado](https://www.dhondt.de/ccx_2.23.src.tar.bz2). Se inspeccionaron dentro del tarball `README.INSTALL`, `CalculiX.h`, `ccx_2.23.c`, `Makefile` y las rutinas `statics.f`, `dynamics.f`, `frequencys.f`, `modaldynamics.f`, `steadystate.c`, `heattransfers.f`, `beamsections.f` y `shellsections.f`. La URL no es content-addressed, por lo que se debe comprobar la huella antes de reutilizarla. |
