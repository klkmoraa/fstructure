...
const COPY = {
  es: {
    title: 'Vistas favoritas', body: 'Guarda preferencias visuales. Seleccionar no modifica el proyecto.', favorite: 'Vista favorita', none: 'No hay vistas guardadas.',
    ...
    storageError: 'No se pudo guardar en este dispositivo.', conflict: 'Ya existe un favorito activo con ese nombre.', invalid: 'Escribe un nombre válido.', applyError: 'No se pudo aplicar la vista.',
  },
  en: {
    title: 'Favorite views', body: 'Save visual preferences. Selecting one does not modify the project.', favorite: 'Favorite view', none: 'No views are saved.',
    ...
    storageError: 'Could not save on this device.', conflict: 'An active favorite already uses that name.', invalid: 'Enter a valid name.', applyError: 'The view could not be applied.',
  },
} as const;
...
export const ViewFavoritesPanel = ({
  ...
  return <section className="member-favorites view-favorites" aria-labelledby="view-favorites-title">
    ...
    <label><span>{copy.favorite}</span><select aria-label={copy.favorite} value={selected?.id ?? ''} onChange={(event) => setSelectedId(event.currentTarget.value)} disabled={!favorites.length}>
      {!favorites.length ? <option value="">{copy.none}</option> : favorites.map((favorite) => <option key={favorite.id} value={favorite.id}>{favorite.name}</option>)}
    </select></label>
