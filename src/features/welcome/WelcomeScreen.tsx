      ...
      : view === 'templates' ? <section className="sc-home-view" aria-label={text.templates}>{heading(text.templatesTitle, text.templatesBody)}<div className="sc-home-template-grid">{visibleExamples.map((example) => {
        ...
        : view === 'library' ? <section className="sc-home-view" aria-label={text.library}>{heading(text.libraryTitle, text.libraryBody)}<PersonalLibraryView language={language} units={project.settings.units} theme={theme} view={readCanvasViewSettings(project)} /></section>
          : view === 'classroom' ? <section className="sc-home-classroom" aria-label={text.classroom}>
            <div className="sc-home-classroom-hero"><div><h2>{text.classroomTitle}</h2><span>{text.classroomBody}</span><button type="button" className="sc-home-continue" onClick={() => openExercise()}>{text.classroomAction}<ArrowRight size={16} /></button></div><ThreeStructuralImage assetId="portal:two-story" theme={theme} eager render="vector" /></div>
            <div className="sc-home-classroom-cases"><h3>{text.classroomCases}</h3><div>{classroomExerciseTemplates.filter((item) => item.id !== 'blank').map((item) => <button key={item.id} type="button" onClick={() => openExercise(item.id)}><ThreeStructuralImage assetId={assetForExercise[item.id]} theme={theme} render="vector" /><strong>{item.name}</strong><span>{item.description}</span><ArrowRight size={16} /></button>)}</div></div>
          </section>
