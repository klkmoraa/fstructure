...
export default function Space3DSurface() {
  ...
  const [sourceVersion] = useState(() => branch?.sourceVersion ?? session?.currentBundle(project.id)?.manifest.sourceVersion ?? crypto.randomUUID());
  const stale = Boolean(branch && branch.sourceVersion !== session?.currentBundle(project.id)?.manifest.sourceVersion);
  ...
  const save = useCallback((model: Space3DProjectV1) => {
    ...
    void session.saveSpace3D(project, linkSpace3DToShell(project.id, sourceVersion, model))
  ...
  return <>
    ...
    <Space3DWorkspace language={project.settings.language} embedded handoff={handoff} storage={embeddedStorage}
      canonicalProject={canonicalProject} onProjectChange={save} />
  </>;
}
