...
import { FilePlus2, FlaskConical, Upload } from 'lucide-react';
...
const content = (onAnalyze: () => void, onImport: (file: File) => void, onNewProject: () => void): ToolHomeContent => ({
  ...
  startBody: { es: 'Tres entradas a la mesa FEM.', en: 'Three ways into the FEM workbench.' },
  paths: [
    { id: 'analyze', icon: FlaskConical, label: { es: 'Analizar el caso de prueba', en: 'Analyse the test case' },
      body: { es: 'Un parche TRI3 con solución conocida para comprobar el solver.', en: 'A TRI3 patch with a known solution to check the solver.' },
      action: onAnalyze },
    ...
    { id: 'new', icon: FilePlus2, label: { es: 'Proyecto nuevo', en: 'New project' },
      body: { es: 'Un proyecto aparte, con su propio estudio FEM.', en: 'A separate project with its own FEM study.' },
      action: onNewProject },
  ],
  capabilities: [
...
export default function FemHome({ onOpenWorkspace, onOpenSuite }: { onOpenWorkspace: () => void; onOpenSuite: () => void }) {
  ...
  return <>
    <ToolHome tool="fem" content={content(analyze, importGmsh, newProject)} summary={summary} onOpenWorkspace={onOpenWorkspace} onOpenSuite={onOpenSuite} />
