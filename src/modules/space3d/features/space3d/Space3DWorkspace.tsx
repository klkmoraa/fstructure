...
const WorkspaceBody = ({
  ...
  const projectSwitcher = <div className="space3d-project-switcher">
    <button
      type="button"
      className="space3d-project-picker"
      aria-label={embedded ? t('space3d.toolbarProject') : undefined}
      aria-expanded={projectMenuOpen}
      onClick={() => setProjectMenuOpen((current) => !current)}
    >
      <span className="space3d-project-picker-copy">
        <strong>{embedded ? t('space3d.toolbarProject') : project.name}</strong>
        {embedded ? null : <small>Solver 3D · Experimental</small>}
      </span>
