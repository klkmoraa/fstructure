 ...
 * looks the file up by `PORTABLE_PAYLOAD_FILENAME`. Its name, MIME type, serialisation and
 ...
 */
import { PORTABLE_PAYLOAD_FILENAME, PORTABLE_PAYLOAD_MIME } from '../portableTypes';
...
export const attachPortablePayload = async (context: ReportContext): Promise<Uint8Array> => {
  ...
  pdf.setTitle(pdfText(`${project.name} - memoria de cálculo FusionStructure`));
  pdf.setAuthor('FusionStructure');
  pdf.setSubject('Modelo, DCL, diagramas N-V-M, resultados y procedimiento estructural');
  pdf.setKeywords(['FusionStructure', 'cálculo estructural', 'DCL', 'NVM', payload.checksum.value]);
  pdf.setProducer(`FusionStructure ${payload.provenance.appVersion ?? ''}`.trim());
  pdf.setCreator('FusionStructure');
  ...
  await pdf.attach(attachment, PORTABLE_PAYLOAD_FILENAME, {
    mimeType: PORTABLE_PAYLOAD_MIME,
    description: 'Proyecto y resultados exactos para reimportación en FusionStructure',
    creationDate: stamp,
